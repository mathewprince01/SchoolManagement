            ...
            <div class="card-body">
                <form action="{{route('teacher.store')}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text" name="full_name" id="full_name" id="full_name" value="{{old('full_name')}}" class="form-control">
                        @error('full_name')
                    ...
                    <div class="mb-2">
                        ...
                        <input type="text" name="qualification" id="qualification" id="qualification" value="{{old('qualification')}}" class="form-control">
                        @error('qualification')
                            <div class="text-danger">{{$message}}</div>
                        @enderror
                    </div>
                    <div class="mb-2">
                        ...
                        <input type="text" name="contact" id="contact" id="contact" value="{{old('contact')}}" class="form-control">
                        @error('contact')
                            <div class="text-danger">{{$message}}</div>
                        @enderror
                    </div>
                     ...
                     <div class="mb-4">
                        ...
                        <select name="assigned_classes[]" id="assigned_classes" class="form-select" multiple>
                            ...
                            @foreach ($classess as $class)
                                <option value="{{$class}}" @selected(in_array($class, old('assigned_classes',[])))>{{$class}}</option>
                            @endforeach
                        </select>
